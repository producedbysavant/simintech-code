...
def test_relative_paths_are_expanded_from_pack_dir_with_forward_slashes(tmp_path):
    ...
    Обратный слэш из `.pak` — **разделитель**, а не часть имени файла. Поэтому
    проверяется, что относительный хвост разложился на два звена и что
    разрешённый путь действительно ведёт к файлу.

    Проверять отсутствие `\\` во **всей** строке пути нельзя, и это не
    придирка: на Windows обратный слэш есть в самом якоре (`C:\\`), поэтому
    такое утверждение невыполнимо ровно там, где библиотека и работает, — оно
    проходило только на Linux и падало в CI на Windows. Признак «разделитель
    разобран» от платформы не зависит: файл находится по разрешённому пути.
    """
    ...
    expected = (tmp_path / "Схемы" / "701.prt").resolve()
    assert resolved[0] == expected
    assert pack.resolve(pack.projects[0]) == resolved[0]
    assert resolved[0].parts[len(tmp_path.resolve().parts):] == ("Схемы", "701.prt"), (
        f"относительный путь не разложился на звенья: {resolved[0].parts}")
    assert resolved[0].is_file(), (
        "разрешённый путь не ведёт к файлу: разделитель из `.pak` не разобран")
