...
# Вложенный объект: контейнер (субмодель) без своих custom_props, внутри —
# блок. Нежадное <object>…</object> обрывалось на вложенном объекте, поэтому
# вложенный блок в разбор не попадал, а его <custom_props> оставались в теле
# контейнера — и параметр ребёнка приписывался классу субмодели, у которого
  ...
  <object>
    ...
    <containerblock>
      <object>
        ...
        <custom_props>
          <data><name>`a`</name><mode>`1`</mode><value>`7`</value></data>
          <data><name>`formula_visible`</name><mode>`0`</mode><value>`0`</value></data>
        </custom_props>
      </object>
    </containerblock>
...
def test_nested_object_is_parsed_and_container_does_not_inherit():
    """Вложенный объект разбирается, а контейнер его параметры не наследует.

    Раньше вложенный блок в разбор не попадал («в файле этого нет» —
    неотличимо от отказа), а его параметры приписывались классу субмодели:
    одному классу отказывали в законном имени, другому разрешали чужое.
    """
    parsed = parse_xprt_block_props(XPRT_NESTED)

    assert parsed["Константа"] == {"a": "7", "formula_visible": "0"}
    assert "Субмодель" not in parsed
    # Объект после контейнера читается по-прежнему
    assert parsed["Усилитель"] == {"a": "2.5"}


def test_nested_computed_param_belongs_to_child_not_container():
    """Вычисляемый параметр ребёнка не становится параметром контейнера."""
    readonly = parse_xprt_readonly(XPRT_NESTED)

    assert readonly["Константа"] == ["formula_visible"]
    assert "Субмодель" not in readonly
