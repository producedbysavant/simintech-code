...
from simintech_api.exceptions import UnsupportedBlockError  # noqa: E402
...
# ─── Отказ на классе, который библиотека не берётся создавать ──────

def test_create_block_refusal_names_the_library_not_com():
    """Отказ называет того, кто отказал, — библиотеку, а не COM.

    Прежнее сообщение утверждало «класс не создаётся через COM CreateBlock», и
    это было **неверно**: `CreateBlock` запись создаёт (измерено 2026-09-18),
    а отказывает библиотека — потому что годность блока в расчёте не проверена.
    Для вызывающего и для агента разница принципиальна: «создать нельзя» и
    «создать можно, но работать не доказано» — разные состояния, и первое
    отправляет искать причину не там.
    """
    page, client = _page()
    ...
    assert "не создаётся через COM" not in text, (
