...
Сборка схемы без расчёта: проект создаётся через `Project.new()` — у такого
проекта нет расчётного слоя, и модельное время в нём не растёт. Выгрузка
переходной характеристики в CSV требует чтения сигнала через COM, которое
сейчас недоступно, поэтому пример честно сообщает об этом и останавливается.
...
def main() -> None:
    ...
    # FindSignalData их не находит. Это закреплено живым тестом
    # test_signal_by_name_refuses_without_signal_base. Список имён
    # получить можно, а читаемое
