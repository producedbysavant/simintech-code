...
prj.set_calc_end_time(5 * RC)     # конечное время расчёта — 5 постоянных
...
sim = prj.simulation()
sim.start()
# run_to не блокирующий: True вернётся только если время действительно дошло
# до отметки (5 постоянных времени — переходный процесс завершён).
reached = sim.run_to(5 * RC)
...
client.disconnect()
if not reached:
    print("Расчёт не дошёл до 5*RC с. Проверьте конечное время расчёта "
