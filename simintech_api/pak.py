...
#: идёт и на Linux, где ``E:\!Work\x.prt`` абсолютным не считается, и запись
...
def _resolve_in(base: Path, project: PackProject) -> Optional[Path]:
    ...
    — она указывает туда, где лежит (например, ``E:\\!Work\\...`` в поставке),
...
class ProjectPack:
    ...
    def resolve(self, project: PackProject) -> Optional[Path]:
        ...
        запись абсолютная (``E:\\!Work\\...``), пустая или уводит вверх за
