...
#: идёт и на Linux, где ``D:\project\model.prt`` абсолютным не считается, и
#: запись молча превратилась бы в относительную.
...
def _pairs(lines: Sequence[str]) -> List[Tuple[str, str]]:
    ...
    Делится по **первому** ``=``: путь вида ``D:\\dir=a\\model.prt``
    встречается, и ``split("=")`` потерял бы хвост пути молча.
    """
...
def _resolve_in(base: Path, project: PackProject) -> Optional[Path]:
    ...
    Разворачиваются только относительные записи: ``Схемы\\model.prt`` →
    ``<каталог пакета>/Схемы/model.prt``. Абсолютной записи разворачивать
    нечего — она указывает туда, где лежит (например, ``D:\\project\\...``),
...
class PackProject:
    ...
    Args:
        ...
        path: путь так, как он записан в файле (``Схемы\\model.prt`` или
            ``D:\\project\\...\\model.prt``).
