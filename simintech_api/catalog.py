...
_OBJECT_TAG_RE = re.compile(r"object(?:_\d+)?\Z", re.I)
...
def _iter_xprt_objects(xml_text: str) -> Iterator["ET.Element"]:
    """Объекты .xprt — все, на любой глубине вложенности.

    Разбор идёт по XML, а не регуляркой: нежадное ``<object>…</object>``
    обрывалось на первом вложенном объекте, из-за чего объект внутри
    контейнера (например блок на странице карты конечного автомата) в разбор
    не попадал вовсе, а ``<custom_props>`` этого объекта оставались в теле
    контейнера — и параметр ребёнка приписывался классу субмодели.

    Текст, который не разбирается как XML (повреждённый файл), даёт пустой
    разбор: неполный разбор — «классов меньше, чем в модели» — неотличим от
    полного, а собрать из обрывков верный каталог всё равно нельзя. Работу с
    повреждённым файлом берёт на себя тот, кто читает его целиком
    (``simintech-mcp`` проверяет корректность XML до разбора).
    """
    try:
    ...
    for element in root.iter():
        if isinstance(element.tag, str) and _OBJECT_TAG_RE.fullmatch(element.tag):
            yield element


def _object_class_name(obj: "ET.Element") -> str:
    """Имя класса объекта — ``<class_name>``, **прямой** ребёнок ``<object>``.

    Ищутся только прямые дети: класс вложенного объекта чужой для контейнера,
    и наследовать его нельзя.
    """
    node = obj.find("class_name")
    return clean_value(node.text if node is not None else None)
...
def parse_xprt_block_props(xml_text: str) -> Dict[str, Dict[str, str]]:
    ...
    for obj in _iter_xprt_objects(xml_text):
        class_name = _object_class_name(obj)
        if not class_name or class_name in NON_BLOCK_CLASSES:
            continue
        custom = obj.find("custom_props")
        if custom is None:
            continue

        props = result.setdefault(class_name, {})
        # Разбор самих записей — по тексту секции: конвенций кавычек и форм
        # `<data>`/`<data_N>` две, и разбирает их одно место (`_iter_data`).
        for prop, value, _mode in _iter_custom_props(
                ET.tostring(custom, encoding="unicode")):
...
def parse_xprt_readonly(xml_text: str) -> Dict[str, List[str]]:
    ...
    for obj in _iter_xprt_objects(xml_text):
        ...
        for prop, _value, mode in _iter_custom_props(
                ET.tostring(custom, encoding="unicode")):
...
def generate_catalog(client: "COMClient",
    ...
    Проект — заведомо пустой (``Project.new()``): здесь не считается, а только
    создаются блоки и снимается выгрузка, поэтому расчётный слой не нужен.
