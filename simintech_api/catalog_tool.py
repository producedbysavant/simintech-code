...
Индекс неполон: базовый слой движка в нём не значится, а часть классов
встречается только вложенными экземплярами (см. `_traversal_classes`), поэтому
к обходу добавляются классы уже собранного каталога — кроме тех, что библиотека
отвергает намеренно (`UNSUPPORTED_COM_BLOCK_CLASSES`): их создание падает
детерминированно и делало бы код возврата ненулевым в любом прогоне.
...
def _known_classes(path: Optional[Path]) -> List[str]:
    """Классы уже собранного каталога — источник имён, которых нет в индексе.

    Отсутствующий файл даёт пустой список (`BlockCatalog.load` на пропавший
    файл отдаёт пустой каталог, а не ошибку), поэтому в чистом checkout
    поведение не меняется.
    """
    return BlockCatalog.load(path).classes()


def _traversal_classes(computed: Optional[List[str]],
                       known: Optional[List[str]] = None) -> List[str]:
    """Классы для обхода: найденные индексом плюс уже известные каталогу.

    Индекс `.csl` неполон, и это свойство поставки, а не недоделка. Базовый
    слой движка («Операция РАВНО», «Оператор И», «Мультиплексор», «Интегратор
    с ограничением», «Язык программирования», ...) в индексе не значится: его
    подключает сам движок. Проверено на SimInTech64 (2026-09-18): строка
    «Операция РАВНО» в `ECS.csl` есть, но это вложенный экземпляр (`Eq5`,
    `calctemplate` `mbty_std.dll` + `TBool` + `logic`), а не запись палитры, и
    `parse_csl_index` отдаёт для этой библиотеки 8 записей без неё. Таких
    классов в поставляемом каталоге 97, из них 16 — эталон работы 25.

    Почему одних имён из индекса мало. Класс, которого нет в обходе, попадает
    в каталог только вложенным экземпляром — а сохранённый экземпляр хранит
    **сокращённый** набор имён. Замер 2026-09-18: прямо созданная «Операция
    РАВНО» отдаёт 7 имён (`log_type`, `true_val`, `false_val`, `T_in`,
    `checkerr`, `eps`, `what`), а её вложенный экземпляр — 5 (`what` первым,
    без `eps`/`checkerr`); ровно этот сокращённый набор и лежал в каталоге до
    ручного досева. Поэтому каталог и служит источником списка: иначе полный
    прогон выбрасывает такие классы целиком, а прямое создание, которое их
    вытесняет, не делается вовсе.

    `computed` = ``None`` (индекс недоступен, `--bin-dir` не задан) означает
    тот же запасной вариант, что и у `generate_catalog`, —
    `SUPPORTED_COM_BLOCK_CLASSES`.

    Классы из `UNSUPPORTED_COM_BLOCK_CLASSES` из обхода вычитаются. Их
    ...
    if computed is None:
        names = set(SUPPORTED_COM_BLOCK_CLASSES)
    else:
        names = set(computed)
...
def main(argv: Optional[List[str]] = None) -> int:
    ...
    if args.xprt is not None:
        from .catalog import decode_xprt, merge_catalogs

        parts = []
        for path in args.xprt:
            # Кодировка выбирается по содержимому (см. `decode_xprt`): старый
            # файл без BOM читается как cp1251, а порча файла с BOM — ошибка.
            # Прежнее `errors="replace"` давало вместо имён классов U+FFFD и
            # сохраняло такой каталог с кодом возврата 0 — порча под видом успеха.
            try:
                text = decode_xprt(path.read_bytes())
            except UnicodeDecodeError as exc:
                print(f"Не удалось декодировать {path}: {exc}",
                      file=sys.stderr)
                return 2
        ...
        # Классы, которые индекс назвать не может, берутся из того каталога,
        # с которым результат будет слит (без `--merge` — из поставляемого):
        # он и есть список классов, для которых каталог обязан остаться цел.
        source = args.merge if args.merge is not None else DEFAULT_CATALOG_PATH
        classes = _traversal_classes(
            _class_names(args.bin_dir, args.profile),
            _known_classes(source))
