...
class Block:
    ...
    def set_name(self, name: str) -> "Block":
        ...
        **Через COM не переименовывает.** `SetBlockProp("Name", …)` —
        документированный no-op: имя остаётся автоматическим (`k_0`, `kx_0`),
        а поиск по запрошенному имени (`Page.find_block`, GUI, разбор `.xprt`)
        ничего не найдёт, причём без ошибки. Фактическое имя берите из
        `get_name()` — по нему же блок адресуется в библиотеке (`connect`,
        сигналы, параметры).
        """
