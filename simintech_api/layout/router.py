"""Расчёт ортогонального маршрута между портами (A* на сетке).

Вход: координаты портов-источника и приёмника + сетка препятствий.
Выход: список опорных точек (x, y) в координатах схемы.

**Это только расчёт.** Геометрию линии задаёт SimInTech (`NormalizeWire`
после `RepaintEditor`), и передать ему свои изломы нечем: `SetWirePoint` на
линии без размеченных точек нормализация игнорирует, а на размеченной даёт
зигзаг (`docs/reference/com_api_inventory.md` §6). Поэтому результат
`route()` нельзя принять за трассировку схемы — она получается расстановкой
блоков (`LayeredPlacer`).

Особенности:
- движение только по 4 направлениям (ортогонально);
- штраф за повороты — маршруты более прямые;
