...
UNSUPPORTED_COM_BLOCK_CLASSES = {
    ...
    "Порт выхода",                # создаётся (id != 0); ранее — регистрация
                                  # порта состояния
    # Библиотека «Конечные автоматы» через COM не создаётся вовсе: CreateBlock
    # возвращает 0 и для карты состояний, и для её детей, и для одиночных
    # классов — в том числе при передаче ParentBlock. Проверено на SimInTech64
    # (2026-09-15, все 8 классов библиотеки). Автомат собирается из доступных
    # блоков: «Ступенька» на моменты переходов + «Сумматор» (см. inventory §5).
    "Карта состояний конечного автомата",
    "Состояние автомата",
    "Вход состояния",
    "Выход состояния",
    "Выдержка состояния",
    "Переменная выдержка состояния",
    "Выход данных состояния",
    "Селектор данных состояния",
