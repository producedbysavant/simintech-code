...
class PortMode(IntEnum):
    """Режим порта (`GetPortInfo`, `SetPortMode`, `getportinfo`).

    Справка знает три значения: 0 — вход, 1 — выход, 2 — ненаправленный, — и
    `SetPortMode` принимает те же три. Режим 2 не экзотика: на выборке из 70
    демо-моделей (1337 портов) он встретился 303 раза — чаще, чем каждый пятый
    порт, — поэтому у него есть имя, а не только «прочее».

    Прежняя редакция комментария называла незадокументированными «универсальный,
    параметр и т.п.»: этих режимов не нашлось ни на одной измеренной модели, а
    `2` задокументирован. `UNKNOWN` — не значение из среды, а признак «такого
    режима мы не знаем»: на него не полагается проба топологии, она отвергает
    разбор.
    """
    INPUT = 0
    OUTPUT = 1
...
# Классы блоков, которые `Page.create_block` отвергает (`UnsupportedBlockError`).
#
# Пометка «через COM не создаются» была неверна. Замерено на живом COM
# 2026-09-18: `CreateBlock` создаёт **обе** записи и возвращает ненулевой id,
# блок появляется на странице, свойства читаются («Из памяти» → `FromMem_0`,
# свойства `a`/`src_type`, портов нет; «Порт выхода» → `PortConnector_0`, один
# порт), и обе записи есть в выгрузке `.xprt`. Не проверено, годятся ли эти
# блоки в модели (расчёт) — поэтому защита оставлена: отказ идёт от библиотеки,
# а не от COM.
UNSUPPORTED_COM_BLOCK_CLASSES = {
    "Из памяти",
    "Порт выхода",  # ранее — регистрация порта состояния
}
