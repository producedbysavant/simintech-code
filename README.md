...
| `simintech_api/core/script_bridge.py` | `ScriptBridge`: выполнить код встроенного языка внутри проекта — то, чего нет в COM (топология, порты и линии связи) |
...
**О тегах.** Их два, и они неравноценны. `v0.2.0` указывает на `44c6af7`, где
`__version__ = "0.1.0"`: в том коде нет ни `standard_block_size`, ни
`default_output_dir`, ни `Project.from_template` — то есть имя тега **новее**
содержимого. `v0.3.0` (`a2daa1c`) согласован: в нём `__version__ = "0.3.0"` и
все три имени на месте. Выпуск версии ставит тег на коммит релиза в `main`.

Осторожность при этом уместна всегда: тег можно передвинуть, и та же строка
зависимости начнёт разрешаться в другой код, причём молча, — в этом репозитории
такой случай уже был, только обратный. Поэтому зависимость объявляйте
**коммитом**:
...
```bash
python3.11 -m pytest tests/unit -q     # весь набор unit-тестов, без COM
