...
```simintech
// Ошибка 4: C-подобные операторы
// if (x == 0) { ... }   — неверно
if x = 0 then begin y = 1.0; end;

// Оператор not поддерживается, но отрицание читается хуже утверждения.
// Обе строки ниже работают — предпочтительнее вторая.
if not flag then begin y = 0.0; end;
