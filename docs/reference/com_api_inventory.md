...
Про переходы: они рисуются объектами-связями — `Связь состояния` (`StateWire`)
и `Связь состояния 2`; оба — графика (точки, стрелки), условия перехода в них
нет. Логику переходов собирает скрипт `bin/include_mvtu/fsm.inc` (генератор
кода автомата, UTF-8+BOM): «состояние» — объект со свойством `state_cod`,
`state_num` — индекс, `isstartstate` — стартовое, `init_cod` — код входа, у
блока-условия — `condition_cod`; результат — `switch(new_state)` с
`curent_state`, `old_state`, `fsm_iter_count`, `storestates`.
