...
**Как задаются переходы** (разобрано по рабочей модели `evs360_simintech`,
`network/logic/task_100ms/FDD003/Mdl_CoolingTT/sub_CoolingTT_FSM.prt`,
2026-09-15):
...
**Связи состояний через COM не создать.** Проверено на SimInTech64
(2026-09-15/16): классы `КА - Связь блоков состояний`, `Связь состояния`,
`Связь состояния 2` через `CreateBlock` возвращают 0, а обычный
`CreateWire` между блоками состояний (то, что делает `Block.connect`) связью
состояний **не становится**: в модели, собранной через API, объектов
`КА - Связь блоков состояний` — 0, тогда как в такой же модели с переходами,
нарисованными в GUI, их по числу переходов. Свойства `to_state_number` /
`from_state_number` у блоков входа/выхода состояния остаются `0` и там и там:
это внутренняя бухгалтерия редактора, а не то, что пишет API-клиент.

Что из этого следует: **карта, состояния, их номера (`self_state_number`,
`is_default`), внутренности состояния и `default_state` у карты собираются
через COM**, а стрелки переходов рисуются один раз в GUI. Дальше такую модель
можно переиспользовать как шаблон (вставлять субмоделью через `LoadSubmodel`)
или дорисовать переходы руками.
