...
`"Height"` — строкой (`'60'`, `'40'`). Отдельного `GetBlockPosition` в API нет,
...
У `Константы`, `Интегратора`, `Усилителя`, `Сумматора` и «В файл» размер,
который создаёт `CreateBlock`, — 60x40, и он же родной: подставлять вместо него
своё значение значит выдать блок нестандартного размера, а это нарушение правил
разработки SimInTech. Обёртка — `Block.get_size()`; `set_position`/`set_center`
сохраняют размер сами.
