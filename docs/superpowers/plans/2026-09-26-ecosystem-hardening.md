...
Локальные чекауты `code` и `skill` совпадают с origin (история обоих —
...
- [ ] **Шаг 1: Написать документ** (текст общий, как у `CONTRIBUTING.md`)
...
- [ ] **Шаг 2: Коммит в каждом репозитории**
...
### Task P1-3: DLP-гейт `public-data-policy`
...
- [ ] **Шаг 5: Добавить обход дерева и CLI**
...
- [ ] **Шаг 6: Прогнать по дереву и разобрать находки**
...
- [ ] **Шаг 9: Перенести скрипт в `simintech-mcp` и `simintech-skill`**
...
- [ ] **Шаг 1: Добавить pre-commit-хук в трёх репозиториях**
...
- [ ] **Шаг 2: Проверить, что запрет файлов работает и не мешает фикстурам**
...
- [ ] **Шаг 3: `tests/fixtures/README.md`** — что здесь допустимо, откуда взят
каждый файл, дата и версия; для `vendor-public/` — ссылка на публичный источник.

### Task P1-5: разовый скан истории

- [ ] **Шаг 1: Прогнать gitleaks по всей истории в трёх репозиториях**
(уже делает CI; здесь — зафиксировать результат на текущем `main`).

- [ ] **Шаг 2: Прогнать DLP-сканер по истории** — `git log -p --all` в файл и
...
- [ ] **Шаг 1: Схема.** Каждое утверждение — запись:
...
- [ ] **Шаг 2: Наполнить начальным набором** — не «все утверждения проекта», а
...
- [ ] **Шаг 3: Валидатор + генератор** (`scripts/evidence.py`):
...
- [ ] **Шаг 4: `sources.md` — список источников** (справка поставки,
...
- [ ] **Шаг 4: Прогнать и убедиться, что реестр и матрица синхронны**
...
- [ ] **Шаг 1: Схема метаданных** (единая для обоих файлов):
...
- [ ] **Шаг 2: Заполнить тем, что известно, остальное — `unknown`**
...
- [ ] **Шаг 3: Тест схемы** — поля присутствуют, `observed_at` либо ISO-дата,
...
- [ ] **Шаг 1: `knowledge-snapshot.md`** — что и на какой версии SimInTech
...
- [ ] **Шаг 2: `compatibility-matrix.md`** — трёхсторонняя таблица:
...
- [ ] **Шаг 3: Сослаться на матрицу из README каждого репозитория** одной
...
### Task P3-1: Windows-nightly с настоящим COM — **нужно решение владельца**

Аудит прав: Linux-CI проверяет разбор и контракты, но живое поведение COM
автоматически не проверяется вообще. План (включать после ответа владельца):
self-hosted runner на Windows-машине с зарегистрированным `mmain.exe /regserver`
и лицензией; workflow `nightly-windows.yml` (`schedule` + `workflow_dispatch`),
который прогоняет `pytest -m integration` и `-m distribution`, сохраняет отчёт
с версиями SimInTech/Python и артефактом результатов.

Вопросы, без которых писать нельзя: доступна ли выделенная машина; допустим ли
self-hosted runner в публичном репозитории (риск: PR из форков на self-hosted
runner — стандартное возражение; решается `workflow_dispatch`/cron без
`pull_request`); как передаётся лицензия SimInTech.

Пока ответа нет — заменить «полуручным контуром»: чеклист живого прогона в
`simintech-code/docs/live-check.md` (что запустить, что записать) и запись
результата в `knowledge-snapshot.md`.
...
P2-1 ──┴─→ P2-2 ─→ P2-3            (code)
P3-*                               (после P0–P2; P3-1 требует решения владельца)
```
...
| P1 | `SECURITY.md` + `DATA_POLICY.md` в трёх репозиториях; job `public-data-policy` зелёный и краснеет на подставном `192.168.x.x`; отчёт по истории отработан |
...
| P3 | решается владельцем; входные условия перечислены в P3-1 |

**Решения, нужные от владельца:**

1. Windows-машина и лицензия для nightly (P3-1) — есть ли, и допустим ли
   self-hosted runner.
2. Канал приёма сообщений об уязвимостях (P1-1): приватный GitHub Advisory
   (рекомендуется) или отдельный адрес.
